function Simple(mes)
{
    document.getElementById("loadingSimple").style.display = "";
    idx = document.getElementsByName("idx")[0].value;
    
    setTimeout(hideLoaderSimple, 5000);
    setTimeout(ShowData, 5500, mes);
}
...
function Simplex()
{
    ...
    document.getElementById("loadingSimple").style.display = "";
    idx = document.getElementsByName("idx")[0].value;
    console.log("idx => simpleX",idx);
    setTimeout(hideLoaderSimple, 5000);
    setTimeout(ShowDataById, 5500, idx);
}

function hideLoaderSimple()
{
    document.getElementById("loadingSimple").style.display = "none";
}
...
function  ShowData(mes)
{
  var array=[];
    ...
    if(mes == 1)
    {
        console.log('array  idx=> BTS Land Contract',array[4])
        ...
        textoMLA = textoMLA +
                    ...
                    "</ul>";
        texto = texto + "<input type='button' class='boton_verde' value='Generate AP' onclick=\"javascript:GenerateAP(" + mes + ")\"/>";
    }
    else
    {
        if(mes == 2)
        {
            console.log('array  idx=> BTS Tenant Contract',array[3])
            textoContract = textoContract + 
                    "<ul>" + 
        ...
        if(mes == 3)
        {
            console.log('array  idx=> Collocation Contract',array[7])
            textoContract = textoContract + 
                    "<ul>" + 
        ...
        if(mes == 4)
        {
            console.log('array  idx=> Collocation Contrac',array[6])
            textoContract = textoContract + 
                    "<ul>" + 
                    "<li>Collocation Contract</li>" +
                    "<li>Site ID: "+ array[6].idx+"</li>" +
                    "<li>Site Number: "+ array[6].siteNumber+"</li>" +
                    "<li>Site Name:  "+ array[6].siteName+"</li>" +
                    "<li>Contact Name:  "+ array[6].contact+"</li>" +
                    "<li>Site Address: "+ array[6].siteAdrres+"</li>" +
                    "<li>Site Type: "+ array[6].siteType+"</li>" +
                    "<li>Antenna Type: "+ array[6].AntenaType+"</li>" +
                    "<li>Heigth:  "+ array[6].heigth+"</li>" +
                    "<li>Rent Amount: "+ array[6].rentAmount+"</li>" +
                    "<li>Latitude:  "+ array[6].latitud+"</li>" +
                    "<li>Longitude:  "+ array[6].longitud+"</li>" +
                    "<li><a href='https://maps.google.com/?q=19.681975,-103.25675' target='_blank'>See on Maps</a></li>" +
                    "</ul>";
            textoMLA = textoMLA +
                    "<ul>" + 
                    "<li>Client:  "+ array[6].client+"</li>" +
                    "<li>Payment frequency: "+ array[6].paymentFrecue+"</li>" +
                    "<li>Date of increment:  "+ array[6].dateIncre+"</li>" +
                    "</ul>";
        }
        texto = texto + "<input type='button' class='boton_verde' value='Generate AP' onclick='javascript:GenerateAP(" + mes + ")'/> &nbsp;" +
                        "<input type='button' class='boton_azul' value='Generate AR' onclick='javascript:GenerateAR(" + mes + ")'/>";
    }
    document.getElementById("infoContractSimple").innerHTML = textoContract;
    document.getElementById("infoMLASimple").innerHTML = textoMLA;
    document.getElementById("actionsSimple").innerHTML = texto;
      })


}

function  ShowDataById(idx)
{
  var array=[];
  var type=null;
  var f = new Date();
   ...
   console.log(f.getDate() + "/" + (f.getMonth() +1) + "/" + f.getFullYear());
    ...
    .then((response) => {
      ...
      .then((data) => {
        ...
        type=array[0].mes;
        console.log('array =>responses',array)
        console.log('array =>responses',array[0].mes)
    ...
    if(type == 1)
    {
        console.log('array  idx=> BTS Land Contract',array[0])
        textoContract = textoContract + 
                    "<ul>" + 
                    "<li>BTS Land Contract</li>" +
    ...
    else
    {
        if(type == 2)
        {
            console.log('array  idx=> BTS Tenant Contract',array[0])
            textoContract = textoContract + 
                    "<ul>" + 
                    "<li>BTS Tenant Contract</li>" +
                    "<li>Site ID:"+ array[0].idx+"</li>" +
